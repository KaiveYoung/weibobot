    ...
    def __getattr__(self, attr):
        try:
            ...
            raise AttributeError(
                r"'JsonDict' object has no attribute '%s'" %
                attr)
...
def _encode_multipart(**kw):
    ...
    for k, v in kw.iteritems():
        ...
        if hasattr(v, 'read'):
            ...
            content = v.read()
            data.append(
                'Content-Disposition: form-data; name="%s"; filename="hidden"' %
                k)
...
class HttpObject(object):
    ...
    def __getattr__(self, attr):
        def wrap(**kw):
            ...
            return _http_call('%s%s.json' % (self.client.api_url, attr.replace(
                '__', '/')), self.method, self.client.access_token, **kw)
        return wrap
...
class APIClient(object):
    ...
    def _parse_access_token(self, r):
        ...
        if remind_in:
            ...
            if rtime < expires:
                expires = rtime
        return JsonDict(access_token=r.access_token, expires=expires,
                        expires_in=expires, uid=r.get('uid', None))
...
class _Executable(object):
    ...
    def __call__(self, **kw):
        ...
        if method == _HTTP_POST and 'pic' in kw:
            method = _HTTP_UPLOAD
        return _http_call('%s%s.json' % (self._client.api_url,
